...
+ ## 心得体会：
  ...
  而我们小组本次的工作，就是创建了`dbf`存储引擎，实现了这些接口，从而实现了表的创建、数据查询、数据删除、数据更新、表重命名和表删除等六个功能。回顾本次期末大作业，我们小组遇到最大的困难并不是编写代码，而是在环境的搭建上。我们小组在项目进行过程中遇到了很多奇怪的BUG，由于自定义MySQL存储引擎的相关工作较少，遇到了问题也无从参考，只能一点一点调试，寻找问题所在；中间还遇到了小组人员变更的情况，一度想要放弃这个课题。不过我们最终还是克服了困难，实现了一个功能较为完善的MySQL存储引擎`dbf`。
